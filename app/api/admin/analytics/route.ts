...
export async function GET(req: NextRequest) {
  ...
  const [
    ...
    supabase.from('page_views').select('*', { count: 'exact', head: true }).gte('created_at', since).eq('country', 'FR').neq('device', 'desktop'),
    supabase.from('page_views').select('path').gte('created_at', since).eq('country', 'FR').neq('device', 'desktop').limit(10000),
    supabase.from('page_views').select('country').gte('created_at', since).eq('country', 'FR').neq('device', 'desktop').limit(10000),
    supabase.from('page_views').select('device').gte('created_at', since).eq('country', 'FR').neq('device', 'desktop').limit(10000),
    supabase.from('page_views').select('utm_source, referrer').gte('created_at', since).eq('country', 'FR').neq('device', 'desktop').limit(10000),
    supabase.from('page_views').select('created_at').gte('created_at', since).eq('country', 'FR').neq('device', 'desktop').limit(10000),
    supabase.from('analytics_events').select('name, created_at').gte('created_at', since).limit(10000),
    supabase.from('page_views').select('path, country, device, created_at').order('created_at', { ascending: false }).eq('country', 'FR').neq('device', 'desktop').limit(20),
    supabase.from('email_clicks').select('*', { count: 'exact', head: true }),
    supabase.from('email_clicks').select('variant_id').order('variant_id').limit(10000),
