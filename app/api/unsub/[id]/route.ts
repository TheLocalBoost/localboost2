...
async function unsubscribe(id: string) {
  const { data } = await sb.from('outreach_links').select('email').eq('id', id).single()
  if (!data?.email) return null
  await sb.from('unsubscribed').upsert({ email: data.email.toLowerCase().trim() }, { onConflict: 'email' })
  return data.email
}
...
export async function GET(req: NextRequest, { params }: { params: Promise<{ id: string }> }) {
  ...
  const email = await unsubscribe(id)
...
// POST — one-click unsubscribe RFC 8058 (Gmail l'appelle automatiquement)
